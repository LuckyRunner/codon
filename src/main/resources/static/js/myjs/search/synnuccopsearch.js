...
function synjunp() {
    ...
    if (a.search("taxonomy_geno=") != -1) {
        ...
        $('#syn_gen_tax').append(newOption).trigger('change');
        return null;
    }
    if (a.search("organism_geno=") != -1) {
        ...
        var b = reg.exec(a);
        var newOption = new Option(b[1], b[1], true, true);
        $('#syn_gen_org').append(newOption).trigger('change');
        return null;
    }
    ...
    if (a.search("gene_id=") != -1) {
        ...
        var newOption = new Option(b[1], b[1], true, true);
        $('#li_gene a').tab('show');
        $('#syn_gen_genid').append(newOption).trigger('change');
        return null;
    }
    if (a.search("assemblyAccession_gene=") != -1) {
        ...
        var newOption = new Option(b[1], b[1], true, true);
        $('#li_gene a').tab('show');
        $('#syn_gen_ass').append(newOption).trigger('change');
        return null;
    }
}
