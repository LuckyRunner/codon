...
function proteinjunp() {
    ...
    if (a.search("taxonomy_geno=") != -1) {
        ...
        $('#pro_gen_tax').append(newOption).trigger('change');
        return null;
    }
    if (a.search("organism_geno=") != -1) {
        ...
        var b = reg.exec(a);
        var newOption = new Option(b[1], b[1], true, true);
        $('#pro_gen_org').append(newOption).trigger('change');
        return null;
    }
    ...
    if (a.search("assemblyAccession_gene=") != -1) {
        ...
        var newOption = new Option(b[1], b[1], true, true);
        $('#li_gene a').tab('show');
        $('#pro_gen_ass').append(newOption).trigger('change');
        return null;
    }
    if (a.search("gene_id=") != -1) {
        ...
        var newOption = new Option(b[1], b[1], true, true);
        $('#li_gene a').tab('show');
        $('#pro_gen_geneid').append(newOption).trigger('change');
        return null;
    }
}
