...
/****
 * @Author:shenkunlin
 * @Description:NucleotideCop业务层接口实现类
 * @Date 2019/6/14 0:16
 *****/
@Service
public class NucleotideCopServiceImpl implements NucleotideCopService {

    @Autowired
    private NucleotideCopMapper nucleotideCopMapper;

    @Override
    public PageInfo<NucleotideCop> findPage(NucleotideCopDTO nucleotideCopDTO, String orderDir, String orderColumn, int draw, int start, int length) {

        Example example = new Example(NucleotideCop.class);
        ...
        if (StringUtils.isNotEmpty(nucleotideCopDTO.getGeneId())&&!StringUtils.equals(nucleotideCopDTO.getGeneId(),"-1")) {
            criteria.andEqualTo("geneId",nucleotideCopDTO.getGeneId());
        }
        if (StringUtils.isNotEmpty(nucleotideCopDTO.getProteinId())&&!StringUtils.equals(nucleotideCopDTO.getProteinId(),"-1")) {
            criteria.andEqualTo("proteinId",nucleotideCopDTO.getProteinId());
        }
        if (nucleotideCopDTO.getLenStart() != null && nucleotideCopDTO.getLenStart()!=-1) {
            criteria.andGreaterThanOrEqualTo("length",nucleotideCopDTO.getLenStart());
        }
        if (nucleotideCopDTO.getLenEnd() != null && nucleotideCopDTO.getLenEnd()!=-1) {
            criteria.andLessThanOrEqualTo("length",nucleotideCopDTO.getLenEnd());
        }
...
//        criteria.andGreaterThan("id",start);
        ...
        return new PageInfo<NucleotideCop>(nucleotideCopMapper.selectByExample(example));
