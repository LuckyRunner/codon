...
/****
 * @Author:shenkunlin
 * @Description:NucleotideCopSynoG业务层接口实现类
 * @Date 2019/6/14 0:16
 *****/
@Service
public class NucleotideCopSynoGServiceImpl implements NucleotideCopSynoGService {

    @Autowired
    private NucleotideCopSynoGMapper nucleotideCopSynoGMapper;

    @Override
    public PageInfo<NucleotideCopSynoG> findPage(NucleotideCopSynoDTO nucleotideCopSynoDTO, String orderDir, String orderColumn, int draw, int start, int length) {

        Example example = new Example(NucleotideCopSynoG.class);
...
//        criteria.andGreaterThan("id",start);
        ...
        return new PageInfo<NucleotideCopSynoG>(nucleotideCopSynoGMapper.selectByExample(example));
    }
}
