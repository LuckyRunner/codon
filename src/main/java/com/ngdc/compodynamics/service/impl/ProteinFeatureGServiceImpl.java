...
/****
 * @Author:shenkunlin
 * @Description:ProteinFeatureG业务层接口实现类
 * @Date 2019/6/14 0:16
 *****/
@Service
public class ProteinFeatureGServiceImpl implements ProteinFeatureGService {

    @Autowired
    private ProteinFeatureGMapper proteinFeatureGMapper;

    @Override
    public PageInfo<ProteinFeatureG> findPage(ProteinFeatureDTO proteinFeatureDTO, String orderDir, String orderColumn, int draw, int start, int length) {

        Example example = new Example(ProteinFeatureG.class);
...
//        criteria.andGreaterThan("id",start);
        ...
        return new PageInfo<ProteinFeatureG>(proteinFeatureGMapper.selectByExample(example));
    }
}
